...
const GlobalStyles = () => (
  <style>{`
    ...
    .nav-tab { font-weight: 700; font-size: 1.1rem; color: #a0a0a0; transition: color 0.2s; text-transform: capitalize; padding: 0.5rem; }
    ...
    .pill-btn-filled:hover { opacity: 0.9; transform: translateY(-2px); }
    .input-field { width: 100%; padding: 1rem 1.5rem; border-radius: 20px; border: 2px solid #eee; background: #f9f9f9; font-size: 1.1rem; font-family: inherit; transition: all 0.2s; outline: none; }
    ...
    @media (max-width: 768px) { .editorial-header { font-size: 2.5rem; } }
    ...
    .modal-backdrop { position: fixed; inset: 0; background: rgba(253,252,249,0.9); backdrop-filter: blur(8px); display: flex; align-items: center; justify-content: center; z-index: 1000; padding: 1rem; overflow-y: auto; }
...
const Navbar = ({ activeTab, setActiveTab, users, setViewedUser, currentUser, onLogout }) => {
  ...
  const filteredUsers = search.trim() ? users.filter(u => u.name.toLowerCase().includes(search.toLowerCase())) : [];
          ...
          <div style={{position: 'relative', zIndex: 100}}>
            <div style={{display: 'flex', alignItems: 'center', background: '#fff', borderRadius: '24px', padding: '0.5rem 1rem', border: '2px solid #eee'}}>
              ...
              <input 
                ...
                style={{border: 'none', outline: 'none', background: 'transparent', marginLeft: '0.5rem', fontFamily: 'inherit', fontWeight: 600, width: '140px'}} 
              />
            </div>
            {showSearch && search.trim() !== '' && (
              <div style={{position: 'absolute', top: 'calc(100% + 0.5rem)', left: 0, right: 0, background: '#fff', borderRadius: '24px', boxShadow: '0 12px 32px rgba(0,0,0,0.1)', overflow: 'hidden', border: '2px solid #eee'}}>
                {filteredUsers.length > 0 ? filteredUsers.map(u => (
                  <div key={u.username} onClick={() => { setViewedUser(u); setShowSearch(false); setSearch(''); }} style={{display: 'flex', alignItems: 'center', padding: '1rem', cursor: 'pointer', borderBottom: '1px solid #f9f9f9', transition: 'background 0.2s'}}>
            ...
            <div style={{position: 'relative', zIndex: 100}}>
              ...
              {showProfile && (
                <div style={{position: 'absolute', top: 'calc(100% + 0.5rem)', right: 0, background: '#fff', borderRadius: '24px', boxShadow: '0 12px 32px rgba(0,0,0,0.1)', overflow: 'hidden', border: '2px solid #eee', minWidth: '160px'}}>
                  <button onClick={() => { onLogout(); setShowProfile(false); }} style={{width: '100%', padding: '1rem', textAlign: 'left', fontWeight: 700, color: '#e53935', display: 'flex', alignItems: 'center', gap: '0.75rem', transition: 'background 0.2s'}}><LogOut size={18}/> Log out</button>
                </div>
              )}
            </div>
          )}
...
const HomeTab = ({ users, setViewedUser, counts }) => (
  <div style={{padding: '2rem'}}>
    ...
    <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '2rem', maxWidth: '1000px', margin: '0 auto'}}>
      {users.map(u => (
      ...
      {users.length === 0 && (
        <div style={{gridColumn: '1 / -1', textAlign: 'center', padding: '4rem', color: '#888'}}>
          ...
          <h3 style={{fontSize: '1.5rem'}}>It's quiet here.</h3>
        </div>
      )}
    </div>
  </div>
...
const CreateProfileForm = ({ users, onSuccess }) => {
  ...
  return (
    <form onSubmit={submit} style={{display: 'flex', flexDirection: 'column', gap: '1.5rem'}}>
      ...
      <div>
        <label style={{fontWeight: 700, display: 'block', marginBottom: '0.5rem'}}>Emoji Avatar</label>
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '0.5rem', background: '#f9f9f9', padding: '1rem', borderRadius: '20px', border: '2px solid #eee', maxHeight: '180px', overflowY: 'auto'}}>
          {EMOJIS.map(e => (
      ...
      <button type="submit" className="pill-btn pill-btn-filled" style={{'--btn-color': '#111', justifyContent: 'center', marginTop: '1rem'}} disabled={!name || !avatar || pin.length !== 4}>Create Profile</button>
    </form>
  )
}
...
const EditProfileForm = ({ currentUser, setPinModalConfig }) => {
  ...
  return (
    <form onSubmit={submit} style={{display: 'flex', flexDirection: 'column', gap: '1.5rem'}}>
      ...
      <button type="submit" className="pill-btn pill-btn-outline" style={{justifyContent: 'center', marginTop: '1rem'}}>Save Changes</button>
    </form>
  )
}
